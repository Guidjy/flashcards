...
def editar_usuario(request, usuario_id):
    try:
        ...
        return Response({'erro': f'Não existe um usuário com esse id ({usuario_id})'}, status=401)
...
def login_usuario(request):
    ...
    try:
        ...
        return Response({'erro': f'Não existe um usuário com esses credenciais'}, status=401)

    return Response({'messagem': 'Login realizado com sucesso.', 'usuario': {'nome': usuario.username, 'id': usuario.id}}, status=200)
...
def criar_deck(request):
    ...
    nome = request.data.get('nome')
    criador = request.data.get('criadorId')
    try:
        criador = User.objects.get(id=criador)
    except User.DoesNotExist:
        return Response({'erro': f'Não existe um usuário com esse id'}, status=401)
...
def editar_deck(request):
    """
    POST:
    deckId: id do deck a editar
    novoNome: novo nome do deck
    """
    deck = request.data.get('deckId')
    novo_nome = request.data.get('novoNome')
...
@api_view(['GET'])
def decks_usuario(request, usuario_id):
    try:
        usuario = User.objects.get(id=usuario_id)
    except User.DoesNotExist:
        return Response({'erro': f'Não existe um usuário com esse id ({usuario_id})'}, status=401)
...
@api_view(['DELETE'])
def deletar_card(request, card_id):  
    try:
...
@api_view(['GET'])
def comecar_estudo(request, deck_id):
    try:
        deck = Deck.objects.get(id=deck_id)
    except Deck.DoesNotExist:
...
def terminar_estudo(request):
    try:
        deck = Deck.objects.get(id=request.data.get('deckId'))
    except Deck.DoesNotExist:
        return Response ({'erro': 'Não existe um deck com esse id'})
